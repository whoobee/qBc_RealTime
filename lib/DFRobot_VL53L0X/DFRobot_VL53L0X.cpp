 ...
 * @brief DFRobot VL53L0X library — local fork patched to use Wire2 on
 *        Teensy 4.1 (SDA2 = pin 25, SCL2 = pin 24). All Wire.* calls were
 *        replaced with Wire2.* so this library speaks only on the secondary
 *        I2C bus. Upstream: github.com/DFRobot/DFRobot_VL53L0X
 */
...
void DFRobot_VL53L0X::writeByteData(unsigned char Reg, unsigned char byte){
	Wire2.beginTransmission(_detailedData.i2cDevAddr);
	Wire2.write(Reg);
	Wire2.write((uint8_t)byte);
...
void DFRobot_VL53L0X::readData(unsigned char Reg, unsigned char Num){
	...
	Wire2.endTransmission();
    Wire2.requestFrom((uint8_t)_detailedData.i2cDevAddr, (uint8_t)Num);

	for(int i=0;i<Num;i++)
	{
		_detailedData.originalData[i] = Wire2.read();
		delay(1);
	}
}
...
uint8_t DFRobot_VL53L0X::readByteData(unsigned char Reg){
	...
	Wire2.write((uint8_t)Reg);
	Wire2.endTransmission();
    Wire2.requestFrom((uint8_t)_detailedData.i2cDevAddr, (uint8_t)1);
	data = Wire2.read();
	return data;
}
